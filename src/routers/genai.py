...
from src.contollers.genai.genai_helpers import regenerate_description, regenerate_query_description
...
@router.post("/generator/text", status_code=status.HTTP_202_ACCEPTED)
async def router_describe_query(
        text: str = Form()
):
    try:
        redis_client = await get_redis_client()
    ...
    existing_query = await redis_client.get(f"text:{text}")
    if existing_query:
        existing_query_id = existing_query.decode('utf-8')
        return JSONResponse(content={"query_id": existing_query_id}, status_code=status.HTTP_200_OK)
    else:
        query_id = str(uuid.uuid4())
        await redis_client.set(f"text:{existing_query}", query_id)

        try:
            cached_description = await redis_client.get(f"description-query:{existing_query}")
        except Exception:
            raise HTTPException(
                status_code=500,
                detail="Cache access error."
        ...
        if cached_description:
            ...
            response = await regenerate_query_description(
                redis_client, text, query_id
            )
            await redis_client.set(f"description-query:{text}", json.dumps(response))
        ...
        for idx, _object in enumerate(item_components):
            ...
            task = generate_image_and_model_task.delay(_object.get("name_to_prompt"), query_id, idx)
            task_ids.append(task.id)

        await redis_client.set(f"status:{query_id}", json.dumps(status_data))
        return JSONResponse(content={"query_id": query_id, "task_ids": task_ids}, status_code=status.HTTP_202_ACCEPTED)
...
async def get_status(id: str):
    try:
        redis_client = await get_redis_client()
        cached_response = await redis_client.get(f"status:{id}")
        if cached_response:
